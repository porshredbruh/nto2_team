...
**Важно:** В бейзлайне для обучения используются только записи с `has_read=1` (книги, которым была поставлена оценка). Записи с `has_read=0` исключаются из обучающей выборки. Однако вполне вероятно, что использование записей с`has_read=0` улучшит значение метрики при правильном использовании.
